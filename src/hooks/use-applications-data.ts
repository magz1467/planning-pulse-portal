...
export const useApplicationsData = () => {
  ...
  const fetchApplicationsInRadius = useCallback(async (
    center: LatLngTuple,
    radius: number,
    page = 0,
    pageSize = 100
    ...
    try {
      ...
      const [appsResponse, countResponse] = await Promise.all([
        ...
        supabase.rpc('get_applications_count_within_radius', {
          center_lng: center[1],
          center_lat: center[0],
          radius_meters: radius
        })
      ...
      // Transform applications in batches for better performance
      const batchSize = 50;
      ...
      for (let i = 0; i < apps.length; i += batchSize) {
        ...
        transformedApplications.push(...batch);
      }
